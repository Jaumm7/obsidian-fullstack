...
> Hora de juntar **TUDO**. Vamos construir, do zero ao deploy local, uma API completa de filmes com **PostgreSQL**, **Entity Framework Core**, **JWT**, **OpenAPI** e **Swagger UI**, exatamente na ordem que profissionais constroem.
>
...
| Campo | Valor |
|---|---|
| **Estrutura (Framework)** | **.NET 10.0 (Suporte de longo prazo)** |
| **Tipo de autenticação** | **Nenhum** (vamos implementar JWT manualmente) |
| **Configurar para HTTPS** | ✅ marcado |
| **Habilitar suporte a OpenAPI** | ✅ marcado (gera o documento OpenAPI; a tela Swagger UI será adicionada por pacote) |
...
> ℹ️ O template do ASP.NET Core 10 normalmente já inclui `Microsoft.AspNetCore.OpenApi` quando você marca suporte a OpenAPI. Mesmo assim, deixar a versão explícita ajuda a padronizar a apostila inteira.
...
2. Abrir o navegador na URL do Swagger UI (`https://localhost:7XXX/swagger`) depois da configuração inicial abaixo.

Se aparecer a tela do Swagger UI, está tudo certo. Pare a execução com **Shift+F5** e siga para o Passo 1 do tutorial (criar `Controllers/HealthCheck.cs`).
...
mkdir backend
cd backend
dotnet new webapi -n Movies.API --use-controllers --framework net10.0
cd Movies.API
```
...
#### Configuração inicial para abrir o Swagger UI

Antes de testar a tela `/swagger`, confira se o `Program.cs` inicial está neste formato. Mais adiante, no Passo 25, vamos substituir este arquivo pela versão completa com JWT, repositórios, services e autenticação:
...
Com isso, o documento OpenAPI fica em `/openapi/v1.json` e a interface interativa fica em `/swagger`. A diferença é importante: o ASP.NET Core 10 gera o documento; o pacote `Swashbuckle.AspNetCore.SwaggerUI` fornece a página de testes.
...
// 2) OpenAPI com suporte a Bearer
...
Neste `Program.cs`, o documento OpenAPI é exposto em `/openapi/v1.json`. A tela `/swagger` vem do pacote `Swashbuckle.AspNetCore.SwaggerUI` e apenas consome esse documento. Como o esquema foi configurado como `type: http` e `scheme: bearer`, o botão **Authorize** espera o token puro; ele mesmo monta o header `Authorization: Bearer <token>`.
...
1. No Swagger UI, clique em **🔒 Authorize** no topo.
2. Cole somente o token: `eyJhbGciOiJIUzI1NiIs...` (sem a palavra `Bearer`).
...
| Swagger UI sem botão "Authorize" | Transformador `BearerSecuritySchemeTransformer` não registrado no `AddOpenApi` | Reveja o `Program.cs` |
| `401 Unauthorized` mesmo com token | Token colado com `Bearer` no Swagger UI, token expirado ou chave diferente | No Swagger UI, cole só o JWT; em clientes manuais, envie `Authorization: Bearer <token>` |
...
- Aplicamos **EF Core + PostgreSQL**, **JWT + OpenAPI + Swagger UI** e **camadas (Model, Mapping, Repository, Service, Controller)**.
- Vimos como testar tudo no Swagger UI.
